...
def _detail_table(findings: list[Any], width: int, multi_project: bool) -> Table:
    """Per-resource rows, laid out for the terminal actually in use.

    Resource identifiers get long -- a KMS key version's path runs past 90
    ...
    The project column appears only when the scan covered more than one, since
    a single-project scan repeats the same value down the page.
    """
    wide = width >= 100
    table = Table(header_style="bold", expand=True)
    if multi_project:
        table.add_column("Project", no_wrap=True, overflow="ellipsis", ratio=2)
    ...
    for finding in findings:
        row = [finding.project] if multi_project else []
        row += [
            finding.location,
            finding.resource_id,
...
def render(
    ...
    project_word = "project" if len(result.projects) == 1 else "projects"
    scope = f"{len(result.projects)} {project_word}"
    ...
    if not result.findings:
        if result.completely_failed:
            ...
            console.print(
                ...
                f"project/check pairs failed. The result below is not an all-clear.\n"
            )
    ...
    console.print()
    console.print(_detail_table(picked, console.width, len(result.projects) > 1))
    if len(picked) < count:
        console.print(
...
def _render_errors(result: ScanResult, console: Console) -> None:
    if not result.errors:
        return
    console.print(
        f"\n[yellow]{len(result.errors)} project/check pair(s) could not be scanned:[/yellow]"
    )
    for error in result.errors[:10]:
        console.print(f"  [dim]{error.project} {error.check}: {error.message}[/dim]")
...
def to_json(
    ...
    return {
        ...
        "scan": {
            ...
            # Pairs where the API was simply switched off on that project. Not
            # failures: a project that has never used Filestore has no
            # Filestore waste.
        ...
        "errors": [
            {"project": e.project, "check": e.check, "message": e.message} for e in result.errors
        ],
    }
...
def to_script(result: ScanResult) -> str:
    ...
    for finding in result.findings:
        lines.append(
            f"# {_comment(finding.project)} {_comment(finding.location)} "
            f"{_comment(finding.resource_id)} — {_comment(finding.reason)}"
        )
