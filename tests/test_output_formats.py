...
def _finding(check="x", rid="r-1", cost=1.0, approx=False, details=None):
    return Finding(
        ...
        reason="because",
        monthly_cost=cost,
        remediation=f"gcloud thing delete {rid}",
...
def test_empty_scan_also_validates():
    jsonschema.validate(to_json(ScanResult(projects=["proj-1"], attempted=1)), SCHEMA)
...
def test_the_projects_scanned_are_recorded(result):
    """A finding's project is meaningless without knowing what was looked at."""
    assert to_json(result)["scan"]["projects"] == ["proj-1", "proj-2"]
...
def test_incomplete_scan_is_flagged_in_the_document(result):
    ...
    dead = ScanResult(projects=["x"], attempted=1)
...
def test_hostile_resource_names_are_escaped():
    ...
    page = to_html(ScanResult(findings=[_finding(rid=evil)], projects=["proj-1"], attempted=1))
...
def test_negligible_total_is_worded_not_printed_as_zero():
    page = to_html(ScanResult(findings=[_finding(cost=0.0)], projects=["proj-1"], attempted=1))
...
def test_print_stylesheet_exists():
    ...
    assert "@media print" in to_html(ScanResult(projects=["proj-1"]))
